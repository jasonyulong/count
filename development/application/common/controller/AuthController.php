...
class AuthController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if (!$this->auth->match($this->noNeedLogin)) {
            ...
            if (!$this->auth->isLogin()) {
                ...
                if (ENVIRONMENT == 'production') {
                    $this->error(__('Please login first'), ERP_DOMAIN . 'login.php', null, 30);
                } else {
                    $this->error(__('Please login first'), url('/count/login/logout', ['url' => $url]), null, 30);
                }
            }
            // 判断是否需要验证权限
        ...
        //渲染菜单
        list($menulist, $navlist, $menus) = $this->auth->getSidebar([], $path);
        $this->view->assign('menulist', $menulist);
        $this->view->assign('navlist', $navlist);

        $this->assign('admin', \rsa\RSA::getInstance()->decrypt(Session::get('admin'))->toArray());
        $this->assign('title', (!empty($menus) ? $menus['title'] : Config::get('site.name')));
