...
namespace app\count\controller\auth;
...
class Rule extends AuthController
{
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if (!isset($params['name']) && empty($params['name'])) $this->error(__('请求地址不能为空'));
            if ($params) {
        ...
        $this->view->assign('menuid', $this->request->get('menuid', 0));
        $this->view->assign('ismenu', $this->request->get('ismenu', 1));
    ...
    {
        $ids = !empty($ids) ? $ids : $this->request->get('0');
     ...
     * @return string
     */
    public function actions($pid = 0)
    {
        $pid = !empty($pid) ? $pid : $this->request->get('0');
        $where = [
            'type' => 'file',
            'pid'  => $pid,
        ];
        ...
        $this->view->assign('actionlist', $list);
        $this->view->assign('menuid', $pid);
        return parent::fetchAuto();
    }
