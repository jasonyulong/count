...
class FMSAuth
{
    ...
    public function check($name, $uid, $relation = 'or', $mode = 'url')
    {
        ...
        foreach ($rulelist as $rule) {
            ...
            if ('url' == $mode && $query != $rule) {
                ...
                if ((in_array($rule, $name) || in_array(trim($rule, '/index'), $name)) && $intersect == $param) {
                    //如果节点相符且url参数满足
                    $list[] = $rule;
                }
            } else {
                if (in_array($rule, $name) || in_array(trim($rule, '/index'), $name)) {
                    $list[] = $rule;
                }
            }
        }
        if (\request()->get('test') == 'auth') {
            print_r($name);
            print_r($list);
            print_r($rulelist);
            print_r($REQUEST);
    ...
    public function getGroups($uid)
    {
        ...
        // 执行查询
        $user_groups = Db::connect('count')->name($this->config['auth_group_access'])
            ->alias('aga')
            ->join('__' . strtoupper($this->config['auth_group']) . '__ ag', 'aga.group_id = ag.id', 'LEFT')
            ->field('aga.admin_id,aga.group_id,ag.id,ag.pid,ag.name,ag.rules')
            ->where("aga.admin_id='{$uid}' and ag.status=1")
            ->select();
    ...
    public function getRuleList($uid)
    {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::connect('count')->name($this->config['auth_rule'])->where($where)->field('id,pid,condition,icon,name,title,ismenu')->order('weigh desc,id asc')->select();
