...
class Tree
{
    ...
    public function init($arr = [], $pidname = NULL, $nbsp = NULL)
    {
        ...
        // 如果有id，将id放在key 的位置上面
        if (count($this->arr) > 0 && isset($this->arr[0]['id'])) {
            $tmp       = $this->arr;
            $this->arr = [];
            foreach ($tmp as $v) {
                $this->arr[$v['id']] = $v;
    ...
    public function getTreeMenu($myid, $itemtpl, $selectedids = '', $disabledids = '', $wraptag = 'ul', $wrapattr = '', $deeplevel = 0)
    {
        ...
        if ($childs) {
            foreach ($childs as $value) {
                ...
                $chaildTpl = '<li class="@class" id="menu-@id"><a href="@url"  url="@url"><i class="@icon"></i> <span>@title</span> </a> @childlist</li>';
                $childdata = $this->getTreeMenu($id, $chaildTpl, $selectedids, $disabledids, $wraptag, $wrapattr, $deeplevel + 1);

                $chaildIds = [];
                $chaildId = $this->getChild($id);
                if ($chaildId) {
                    $chaildIds  = array_column($chaildId, 'id');
                }
                $ulstyle = '';
                if ($childdata) {
                    $ulstyle = in_array(intval($selectedids), $chaildIds) ? 'style="display: block;"' : '';
                }

                $childlist = $childdata ? "<{$wraptag} {$wrapattr} {$ulstyle}>" . $childdata . "</{$wraptag}>" : "";
